...
for i in $(seq 1 $NODE_NUM); do
    ...
    # ssh "$ssh_user_host" -- "docker-compose exec adkg bash -c 'cd /usr/src/adkg && pip uninstall -y --break-system-packages pypairing && pip install --break-system-packages --no-binary :all: --no-cache-dir --force-reinstall ./pairing'"
